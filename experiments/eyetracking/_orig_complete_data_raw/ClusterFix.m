function [fixationstats] = ClusterFix(eyedat,samprate,reclustering)
...
debugMessage=false;
emptyAction='singleton'; %'singleton','drop',or 'error'
if ~exist('reclustering','var')
    reclustering=true;
end
...
for cndlop = 1:length(eyedat)
    if size(eyedat{cndlop},2) > 500/samprate/1000
        ...
        x = [x(buffer:-1:1) x x(end:-1:end-buffer+1)]; %add buffer for filtering
        y = [y(buffer:-1:1) y y(end:-1:end-buffer+1)]; %add buffer for filtering
        ...
        for numclusts = 2:5
            ...
            T = kmeans(points(1:10:end,2:4),numclusts,'replicate',5,'EmptyAction',emptyAction);
            [silh] = InterVSIntraDist(points(1:10:end,2:4),T);
            sil(numclusts) = mean(silh);
        end
        sil(sil > 0.9*max(sil)) = 1;
        numclusters = find(sil == max(sil));
        T = kmeans(points,numclusters(end),'replicate',5,'EmptyAction',emptyAction);
        ...
        if reclustering
            ...
            for ii =1:(size(fixationtimes,2)); %BUG ON THE LAST CLUSTER, SKIP FOR NOW
                
                if debugMessage
                    fprintf('----Reclustering Trial %d - Clusted %d / %d\n',cndlop,ii,size(fixationtimes,2))
                end
                ...
                for numclusts = 1:5
                    ...
                    T = kmeans(POINTS(1:5:end,:),numclusts,'replicate',5,'EmptyAction',emptyAction);
                    [silh] = InterVSIntraDist(POINTS(1:5:end,:),T);
                    sil(numclusts) = mean(silh);
                end
                sil(sil > 0.9*max(sil)) = 1;
                numclusters = find(sil == max(sil));  %it's dangerous to have too many clusters
                T = kmeans(POINTS,ceil(median(numclusters)),'replicate',5,'EmptyAction',emptyAction);
