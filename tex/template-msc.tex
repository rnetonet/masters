...
Os métodos de detecção supervisionados/explícitos necessitam que o rótulo correto de cada exemplo processado seja informado, o que os torna inviáveis, por causa do custo e do tempo para obtenção de cada rótulo.
Enquanto as técnicas não supervisionadas/implícitas têm dificuldade para antender às restrições de tempo de execução e de uso dos recursos computacionais.
...
A camada intermediária, através de funções de base radial, realiza uma transformação não-linear dos dados para um espaço com alta dimensionalidade, a fim de tornar o problema linearmente separável \cite{cover1965browse}.
...
A camada intermediária das redes RBF forma, implicitamente, grupos (\textit{clusters}) no espaço de alta dimensionalidade criado.
Neste projeto de trabalho de mestrado, esta propriedade é utilizada para propôr um novo método de detecção de mudanças de conceito em fluxos contínuos de dados.
A criação de novos centros ou alteração do centro ativo sinalizam a possível ocorrência de mudanças de conceito.

Para exemplificar de forma simplificada e intuitiva a execução desta proposta de mestrado, considere o conjunto $S = {}$ como um recorte dos dados recepcionados a partir de um fluxo contínuo de dados.
