...
Espera-se que com a utilização de redes RBF, seja possível detectar mudanças de conceito reais em fluxos contínuos de dados em tempo de execução, de forma computacionalmente eficiente e independente de rótulos.
...
Essas técnicas precisaram ser adaptadas para contextos com fluxos contínuos de dados, pois estes cenários apresentam severas restrições de tempo de execução e de uso dos recursos computacionais.
...
Para mitigar este problema, técnicas de detecção de mudanças de conceito foram propostas. 
Estes métodos identificam com precisão o momento da mudança, permitindo que o modelo de decisão seja atualizado somente quando necessário.

Entretanto, as técnicas de detecção propostas apresentam limitações ao serem aplicadas em cenários com fluxos contínuos de dados.
...
Neste trabalho de mestrado, esta propriedade é utilizada para propôr um novo método de detecção de mudanças de conceito em fluxos contínuos de dados, 
baseado no monitoramento do centro ativo.

O algoritmo projetado utiliza apenas as camadas inicial e intermediária da arquitetura de uma rede RBF simplificada.
Além disso, a técnica utiliza uma função gaussiana (Equação \ref{eq:gaussiana}) para ativação e requer a definição de dois parâmetros: 
\texttt{sigma}, utilizado na função radial e responsável por limitar o raio do agrupamento, e \texttt{threshold}, que define o valor mínimo para ativação de um centro.
...
Para este exemplo, os parâmetros foram definidos de forma empírica. O parâmetro \texttt{sigma} foi definido com o valor $0.2$ e o parâmetro \texttt{threshold} foi fixado em $0.6$.
A seguir, o funcionamento da técnica proposta ao tratar o conjunto $S$ é descrito. A Figura \ref{fig:funcionamento_algoritmo} apresenta esse comportamento graficamente.
...
Em $T2$ e $T3$, respectivamente, são recebidos os valores $0.12$ e $0.13$ que são imediatamente vinculados ao centro atualmente ativo ($0.11$) através da função de base radial gaussiana.

No instante $T4$, o valor $0.34$ não atinge o \textit{threshold} parametrizado para o centro ativo. Dessa forma, o valor é definido como um novo centro e ativado.
A ocorrência de mudança de conceito é sinalizada.

Os instantes $T5$, $T6$, $T7$ e $T8$ são vinculados ao último centro ativo ($0.34$). Contudo, em $T9$ o centro $0.11$ volta a ser ativo, pois apresentar maior valor de ativação. Logo, uma nova mudança de conceito é sinalizada.
Finalmente, no instante $T10$ o valor é vinculado ao centro $0.11$.


% % Please add the following required packages to your document preamble:
% % \usepackage{graphicx}
% \begin{table}[ht]
%     \label{tbl:funcionamento_algoritmo}
%     \centering
%     \resizebox{\textwidth}{!}{%
%     \begin{tabular}{|c|c|c|c|c|c|}
%     \hline
%     \textbf{Instante} & \textbf{Valor Recebido} & \textbf{Centros Estabelecidos} & \textbf{Centro Ativado} & \textbf{Valor de Ativação} & \textbf{Descrição}                                                                 \\ \hline
%     T1                & $0.11$                  &                                & $0.11$                  & -                          & Por não haver nenhum centro estabelecido, $0.11$ é definido como centro e ativado. \\ \hline
%     T2                & $0.12$                  &  $\{0.11\}$                    & $0.11$                  & $1.00$                     & Valor vinculado ao centro ativo.                                               \\ \hline
%     T3                & $0.13$                  &  $\{0.11\}$                    & $0.11$                  & $1.00$                     & Valor vinculado ao centro ativo.                                               \\ \hline                      
%     T4                & $0.34$                  &  $\{0.11\}$                    & $0.34$                  & -                          & Valor não foi ativado para nenhum centro ($< threshold$). Novo centro é criado e ativado. Mudança de conceito é identificada.                                               \\ \hline
%     T5                & $0.45$                  &  $\{0.11, 0.34\}$              & $0.34$                  & $0.86$                     & Valor vinculado ao centro ativo.                                               \\ \hline                                            
%     T6                & $0.47$                  &  $\{0.11, 0.34\}$              & $0.34$                  & $0.81$                     & Valor vinculado ao centro ativo.                                               \\ \hline
%     T7                & $0.33$                  &  $\{0.11, 0.34\}$              & $0.34$                  & $1.00$                     & Valor vinculado ao centro ativo.                                                                                   \\ \hline
%     T8                & $0.25$                  &  $\{0.11, 0.34\}$              & $0.34$                  & $0.90$                     & Valor vinculado ao centro ativo.                                                   \\ \hline
%     T9                & $0.14$                  &  $\{0.11, 0.34\}$              & $0.11$                  & $0.99$                     & Valor é vinculado ao centro anterior, por ter maior valor de ativação.Este centro passa a ser o ativo. Mudança de conceito é identificada.                                                                                   \\ \hline
%     T10               & $0.10$                  &  $\{0.11, 0.34\}$              & $0.34$                  & $1.00$                     & Valor vinculado ao centro ativo.                                                                                    \\ \hline
%     \end{tabular}%
%     }
%     \caption{Funcionamento do algoritmo para o fluxo $S$}
% \end{table}

\begin{figure}[H]
\begin{center}
    \noindent\makebox[\textwidth]{\includegraphics[width=\textwidth]{imagens/funcionamento_algoritmo.png}}
    \caption{Exemplo de funcionamento do algoritmo}
    \label{fig:funcionamento_algoritmo}
\end{center}
\end{figure}

Por fim, o algoritmo proposto é descrito, de forma simplificada e reduzida, em pseudocódigo:

\begin{algorithm}[h]
    \SetAlgoLined
    \Entrada{$valor, \sigma, threshold$} 
    \Saida{Booleano indicando a ocorrência ou não de mudança de conceito}
    \Inicio{
     $mudanca \leftarrow falso$\\
     \Para{cada \texttt{centro}}{
         $ativacao \leftarrow gaussiana(valor, \sigma)$\\
         \uIf{$ativacao > threshold$}{
            marque \texttt{centro} como ativo\\
            $threshold \leftarrow ativacao$\\
         }
      }
