...
Por exemplo, podemos considerar como mudança de conceito quando uma pessoa passa a preferir um novo gênero cinematográfico.
...
\begin{enumerate}
    ...
    contudo, estas mudanças não têm impacto no conceito alvo (representado na Figura \ref{fig:real_and_virtual_concept_drift} (b)).
    \item Mudanças de conceito reais referem-se a mudanças na relação $p(y|X)$ e afetam o conceito alvo (representado na Figura \ref{fig:real_and_virtual_concept_drift} (c)).
\end{enumerate}
...
Suponha, que à época, o usuário está buscando um novo apartamento. 
...
pois os limites de decisão do modelo corrente estão obsoletos em relação aos novos exemplos \cite{Gama:2014:SCD:2597757.2523813}.
...
Conforme ilustrado em \ref{fig:concept_drift_patterns}, mudanças de conceito podem ocorrer de forma abrupta, gradual, incremental ou recorrente.
...
Por exemplo, João, ao invés de subitamente passar a preferir livros de auto-ajuda, adquire interesse nestes ao longo do tempo.
...
de ficção científica e aventura para aventura e suspense, de aventura e suspense para suspense a auto-ajuda, e finalmente de suspense e auto-ajuda para apenas auto-ajuda.
...
De volta a nosso exemplo, João está atualmente interessado em livros de ficção científica.
...
Um dos principais desafios para as técnicas de detecção de mudança de conceito é conseguir diferenciar mudanças de conceito reais de \textit{outliers} ou ruídos.
Esta diferenciação é relevante, pois nenhuma adaptação do modelo é necessária quando um \textit{outlier} ou ruído é detectado em um fluxo de dados \cite{Gama:2014:SCD:2597757.2523813}.
Para evitar alarmes falsos, métodos para detecção de \textit{outliers} e ruídos podem ser utilizados em conjunto com os métodos de detecção de mudança de conceito.
