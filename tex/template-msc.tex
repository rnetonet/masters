...
\approvalsheet{Salvador, 17 de Maio de 2019}{
...
A quantidade de dados produzidos por sistemas computacionais tem crescido de forma exponencial nos últimos anos.
Parte relevante dessas informações é produzida na forma de fluxos contínuos de dados, que são sequências ininterruptas e potencialmente infinitas.
Pesquisadores têm aplicado técnicas de aprendizado de máquina para extração de informações úteis desses fluxos.
Contudo, as sequências produzidas são, em sua maioria, não-estacionárias, podendo sofrer variações no contexto do processo gerador ou na distribuição dos dados.
...
Objetivando mitigar este problema, foram desenvolvidas novas técnicas para detecção de mudanças de conceito.
Entretanto, estas técnicas apresentam limitações ao serem aplicadas em cenários com fluxos contínuos, como a necessidade de rotulação por especialistas ou a incapacidade de atender às restrições de tempo de processamento e de uso dos recursos computacionais.
Neste trabalho de mestrado, busca-se desenvolver uma novo método de detecção baseado em redes de função de base radial.
...
O algoritmo desenvolvido nesta pesquisa monitora o centro ativo desse agrupamento e detecta mudanças de conceito quando este é alterado.
O método proposto se diferencia dos trabalhos presentes na literatura por ser computacionalmente eficiente, independente de rótulos e apresentar precisão equivalente ao estado da arte.
