...
O \textit{MOA – Massive Online Analysis} é o principal framework para mineração de dados em fluxos de dados e apresenta uma comunidade crescente e ativa.
O framework contempla uma coleção de algoritmos de aprendizado de máquina (AM): classificação, regressão, agrupamento, busca por padrões, detecção de \textit{outliers}, detecção de mudanças de conceito e sistemas de recomendação.
Além de conter ferramentas para avaliação destes.
Apresenta relação com o projeto WEKA \cite{Hall:2009:WDM:1656274.1656278}, sendo também desenvolvido em Java, apesar de ter uma arquitetura mais voltada para escalabilidade.
...
controlar suas execuções e a visualizar os resultados parciais.
...
O MOA é capaz de ler arquivos \textit{ARFF}, muito utilizados graças ao projeto WEKA \cite{Hall:2009:WDM:1656274.1656278}.
...
Além destes geradores, a aplicação possibilita a obtenção de novos fluxos através da associação ou aplicação de filtros à sequências existentes.
...
A janela de configuração para o detector (similar a \ref{fig:moa_detector}) é criada dinamicamente, através dos atributos declarados na classe.
