...
este projeto de mestrado visa comprovar a hipótese que o monitoramento destas alterações permite detectar mudanças de conceito em fluxos contínuos de dados em tempo de execução, de forma computacionalmente eficiente e independente de rótulos.
...
\begin{algorithm}[h]
    ...
    \Inicio{
     ...
     $centroAtivo \leftarrow null$\\
     $centroAnterior \leftarrow null$\\
    ...
    \uIf{$centroAtivo == null$}{
        $centroAnterior \leftarrow centro$\\
        $centroAtivo \leftarrow valor$ \\
        $centros \leftarrow valor$ \\
    }   
    \uIf{$centroAnterior <> centroAtivo$}{
...
A segunda atividade planejada neste cronograma foi realizada durante o curso da disciplina MATE65 -- Fundamentos de Pesquisa em Ciência da Computação I 
e nos meses subsequentes, 
conforme orientações do orientador.
...
Nesta parte, fluxos contínuos de dados sintéticos com mudança de conceito foram criados e analisados conforme apresentado no Capítulo \ref{experimentos_iniciais}. 
