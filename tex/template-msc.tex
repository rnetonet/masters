...
Entretanto, os métodos propostos apresentam limitações ao serem aplicados em cenários com fluxos contínuos, como a necessidade de rotulação por especialistas ou a incapacidade de atender às restrições de tempo de processamento e de uso dos recursos computacionais desses cenários.
Este projeto de trabalho de mestrado propõe um novo método de detecção baseado em redes de função de base radial.
As redes RBF são utilizadas pois realizam, em sua camada intermediária, uma transformação não-linear que, implicitamente, produz agrupamentos no plano de alta dimensionalidade criado.
O algoritmo desenvolvido, denominado RBFDriftDetector, monitora o centro ativo desse agrupamento e detecta mudanças de conceito quando este centro é alterado.
...
Uma Rede de Função de Base Radial (redes RBF, \textit{radial basis function, RBF network}), pode ser definida como um modelo de múltiplas camadas alimentadas adiante (\textit{feedforward}), 
...
Este capítulo descreve como o trabalho de mestrado será desenvolvido.
...
Contudo, as adaptações propostas não tratam modificações na distribuição dos dados ou no contexto do processo gerador do fluxo.
Estas modificações são denominadas mudanças de conceito e podem afetar negativamente a acurácia do algoritmo \cite{Gama:2014:SCD:2597757.2523813}.
...
Enquanto as técnicas não supervisionadas/implícitas têm dificuldade para atender às restrições de tempo de execução e de uso dos recursos computacionais desses cenários.
...
As redes RBF podem ser definidas como redes neurais multicamadas, alimentadas adiante, capazes de analisar padrões complexos e resolver problemas não-linearmente separáveis \cite{Braga:RedesNeuraisTeoriaAplicacoes}.
...
Isto ocorre, pois a camada intermediária forma, implicitamente, grupos (\textit{clusters}) no espaço de alta dimensionalidade criado.
...
\textit{$\sigma$}, responsável por limitar o raio da radial, e \textit{$\lambda$}, que define o valor mínimo para ativação de um centro.
...
Antes de implementar o algoritmo proposto nesta pesquisa, investigou-se a aplicação de técnicas para detecção de \textit{changing points} na detecção de mudanças de conceito.
Para tanto, foi realizada a implementação no framework MOA de um método de detecção baseado no teste estatístico não-paramétrico de Pettit \cite{Pettitt}.

Para obter familiaridade com a arquitetura do framework MOA e verificar se métodos para detecção de \textit{changing points} podem ser aplicados ao problema de mudança de conceito, 
foi realizada a implementação do método de Pettitt.

Uma das tarefas executadas durante a elaboração deste projeto, consistiu na implementação do método de Pettitt \cite{Pettitt} na plataforma MOA.
A tarefa tinha dois objetivos: familirizar-se com a arquitetura do framework e testar a aplicabilidade de técnicas \textit{changing point} ao problema de detecção de mudanças de conceito.
...
Quanto a performance, o algoritmo proposto obteve a segunda melhor média em tempo de processamento.
O algoritmo ADWIN obteve, por um pequena margem, o melhor tempo médio de processamento.
...
Conforme os resultados, pode-se verificar que o algoritmo proposto neste trabalho identificou todas as mudanças de conceito existentes no fluxo sem indicar nenhum falso-positivo, sendo também o algoritmo com menor atraso de detecção.
