...
O método foi implementado para o framework MOA\footnote{\url{https://git.io/fjGuv}} e o seu pseudocódigo é listado em Algoritmo \ref{alg:algoritmo}.
...
\begin{algorithm}[H]
    ...
    \Inicio{
     $centros \longleftarrow ()$, $centroAtual \longleftarrow null$, $centroAtivo \longleftarrow null$, $mudanca \longleftarrow falso$\\

     \ParaTodo{centro}{
         $ativacao \longleftarrow gaussiana(valor, \sigma)$\\
         \Se{$ativacao >= \lambda$}{
    ...
    \Se{$centroAtual == null$}{
        $centroAtual \longleftarrow valor$ \\
    }
    \Se{$centroAtual \neq centroAtivo$}{
...
Com o objetivo de aproximar os dados produzidos da realidade, foi necessário alterar a classe\footnote{\url{https://git.io/fjGEj}} para permitr a adição de um ruído randômico para cada exemplo produzido.
...
A classe modificada foi utilizada para produzir um terceiro conjunto de dados, com ruído entre $[-0.1, 0.1]$ e sem mudanças de conceito, utilizado como \textit{baseline}.

\section{Critérios de avaliação}

Para avaliação do algoritmo proposto neste trabalho e comparação com o estado da arte, será utilizada a classe \textit{BasicConceptDriftPerformanceEvaluator} do MOA.
Esta classe permite avaliar a performance de métodos de detecção de mudanças de conceito, analisando dimensões de acurária e eficiência computacional.
A classe de avaliação deve ser executada através da tarefa \textit{EvaluateConceptDrift}, disponível na aba \textit{Concept Drift} da tela inicial da aplicação.
A tela de configuração da tarefa é demonstrada na Figura \ref{fig:evaluateconceptdrift_task}.

O restante desta subseção descreve as configurações aplicadas à classe \textit{BasicConceptDriftPerformanceEvaluator} para realização dos experimentos e as dimensões analisadas.

\begin{figure}[H]
    \begin{center}
        \includegraphics[width=\textwidth]{imagens/evaluateconceptdrift_task.png}
        \caption{Parametrização da tarefa \textit{EvaluateConceptDrift}}
        \label{fig:evaluateconceptdrift_task}
    \end{center}
\end{figure}

O parâmetro \textbf{learner} foi definido como \textit{ChangeDetectorLearner} e o seu parâmetro, \textbf{driftDetectionMethod}, configurado conforme a técnica aplicada durante o experimento.

O campo \textbf{stream} define a fonte de dados a ser utilizada para a avaliação e foi configurado como uma fonte \textit{ARFFFileStream}, vinculada a um dos arquivos \textit{ARFF} descritos na subseção anterior.
O parâmetro \textbf{classIndex}, da classe \textit{ARFFFileStream}, é setado para $0$, pois as fontes de dados utilizadas neste trabalho não contém rótulos definidos.

Os parâmetros \textbf{instanceLimit} e \textbf{timeLimit}, que restringem, respectivamente, as quantidades de instâncias a serem processadas e o tempo de execução total, são configuradas como $-1$, desabilitando essas limitações.

Por fim, o campo \textbf{sampleFrequency} é definido como $1$,  obrigando o MOA a gerar uma linha de resultado para cada instância processada.

As principais dimensões analisadas pela classe de avaliação \textit{BasicConceptDriftPerformanceEvaluator} e os seus significados são apresentados na Tabela \ref{tbl:dimensoes_avaliador_drift}:
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|c|}
    \hline
    \textbf{Dimensão} & \textbf{Significado} \\ \hline
                      &                      \\ \hline
                      &                      \\ \hline
    \end{tabular}%
    %}
    \label{tbl:dimensoes_avaliador_drift}
\end{table}
