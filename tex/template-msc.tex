...
\begin{center} 
    ...
    \midrule
    Tempo de Processamento    &  Tempo médio, em segundos, de processamento para cada exemplo. \\
    Mudanças Existentes       &  Quantidade de mudanças existentes. \\
    Mudanças Detectadas       &  Quantidade de mudanças detectadas. \\
    % \textit{Mudanças Reais Detectadas} &  Quantidade de mudanças detectadas corretamente. \\
    Atraso de Detecção        &  Quantidade média de instâncias necessárias para detectar uma mudança. \\
    \bottomrule
    \end{tabular}
    }
    \end{table}
...
\begin{center} 
    ...
    CUSUM                     &  $\textit{MinNumInstances} = 30; \delta = 0.005; \lambda = 50$ \\
    PageHinkley               &  $\textit{MinNumInstances} = 30; \delta = 0.005; \lambda = 50; \alpha = 1$ \\
    ADWIN                     &  $\delta = 0.002$ \\
    \bottomrule
    \end{tabular}
    }
...
\begin{center} 
    ...
    \begin{tabular}{lllll}
    \toprule
    Algoritmo & Tempo de processamento & Mudanças Existentes & Mudanças Detectadas & Atraso de Detecção \\
    \midrule
    RBFDriftDetector          &  $0.28$ & $6$ & $6$ & $1$ \\
    CUSUM                     &  x    & 6 & x & x \\
    PageHinkley               &  x    & 6 & x & x \\
    ADWIN                     &  x    & 6 & x & x \\
    \bottomrule
