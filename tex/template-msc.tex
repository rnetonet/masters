...
Na última década, tem havido um grande interesse de pesquisa nas aplicações dessas técnicas para diferentes domínios.
...
% Comitês de classificadores também foram utilizados para detecção de mudanças de conceito.
% Esses comitês formam a predição final a partir da combinação das predições individuais de cada classificador. 
% Divergências identificadas durante o processo de combinação indicam possíveis mudanças de conceito \cite{Gama:2014:SCD:2597757.2523813}.
% São exemplos de aplicação desta técnica:
...
Visando resolver essas limitações, este projeto de mestrado propõe um algoritmo baseado em redes de função de base radial capaz de detectar mudanças de conceito em fluxos contínuos de dados, de forma online, precisa e independente de exemplos prévios ou rótulos.
