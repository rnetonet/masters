...
Para este trabalho, a classe \texttt{BasicConceptDriftPerformanceEvaluator} é mais relevante, por realizar a análise de  algoritmos para detecção de mudança de conceito.
...
\texttt{$\sigma$}, responsável por limitar o raio da função gaussiana, e \texttt{$\lambda$}, que define o valor mínimo para ativação de um centro.
...
Para este exemplo, os parâmetros foram definidos de forma empírica. O parâmetro \texttt{$\sigma$} foi definido com o valor $0.2$ e o parâmetro \texttt{$\lambda$} foi fixado em $0.6$.
...
No instante $T4$, o valor $0.34$ não atinge o valor mínimo de ativação (\texttt{$\lambda$}) para ser vinculado ao centro ativo ($0.11$). 
...
Este capítulo apresenta um conjunto de experimentos preliminares realizados sobre dois fluxos contínuos sintéticos, 
cujo objetivo principal foi demonstrar a viabilidade da proposta deste projeto de mestrado.
A próxima seção apresenta como cada fluxo sintético foi produzido para condução dos experimentos.

\section{Configuração dos Experimentos}

Os fluxos contínuos de dados sintéticos utilizados nos experimentos foram criados através de classes geradoras do MOA \cite{Bifet:2010:MMO:1756006.1859903}.
Para que pudessem ser reutilizados entre os testes, os dados gerados foram salvos em arquivos no formato ARFF \cite{Hall:2009:WDM:1656274.1656278} através da tarefa \texttt{WriteStreamToARFFFile}.

Foram geradas 10.000 observações para cada conjunto de dados, com valores entre 0 e 1.
Cada conjunto contempla dois conceitos distintos.

Para produzir o fluxo representando mudanças abruptas de conceito, foi utilizada a classe \texttt{AbruptChangeGenerator}.
Esta classe foi alterada para permitir a parametrização de valores mínimos e máximo de ruído a serem aplicados aos dados gerados.
Pois, não existia, em sua versão orginal, a possibilidade de incluir algum ruído aos dados produzidos.
