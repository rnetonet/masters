...
A classe de avaliação deve ser executada através da tarefa \textit{EvaluateConceptDrift}, disponível na aba \textit{Concept Drift}, presente na tela inicial da aplicação.
...
Por fim, o campo \textbf{sampleFrequency} é definido como $1$,  indicando ao MOA que cada instância processada deve gerar uma linha de resultado.
As principais dimensões analisadas nestes resultados são apresentados na Tabela \ref{tbl:dimensoes_avaliador_drift}:
