...
Considerando que o centro ativo desse agrupamento sofre alterações ao longo do tempo, influenciado pelo valor de entrada e o resultado da sua transformação não-linear,
este projeto de mestrado visa comprovar a hipótese que o monitoramento da ativação de centros permite detectar mudanças de conceito em fluxos contínuos de dados em tempo de execução, de forma computacionalmente eficiente e independente de rótulos.
...
\texttt{$\sigma$}, responsável por limitar o raio da função gaussiana, e \texttt{$\epsilon$}, que define o valor mínimo para ativação de um centro.
...
Para este exemplo, os parâmetros foram definidos de forma empírica. O parâmetro \texttt{$\sigma$} foi definido com o valor $0.2$ e o parâmetro \texttt{$\epsilon$} foi fixado em $0.6$.
...
Em $T2$ e $T3$ são recebidos, respectivamente, os valores $0.12$ e $0.13$ que são imediatamente vinculados ao centro atualmente ativo ($0.11$), pois seus valores de ativação foram maiores que $0.6$ ($\epsilon$).

No instante $T4$, o valor $0.34$ não atinge o valor mínimo de ativação (\texttt{$\epsilon$}) para ser vinculado ao centro ativo corrente ($0.11$). 
...
Finalmente, o valor do instante $T10$ é vinculado ao centro $0.11$.

A Figura \ref{fig:funcionamento_algoritmo} apresenta esse comportamento graficamente. 
Os círculos na cor amarela foram ativados para o centro $0.11$ ($T1$), enquanto os de cor magenta foram ativados para o centro $0.34$ ($T4$).
...
Por fim, o algoritmo proposto é descrito, de forma simplificada, em pseudocódigo:

\begin{algorithm}[h]
    \SetAlgoLined
    \Entrada{$valor, \sigma, \epsilon$} 
    ...
    \Inicio{
     ...
     \Para{cada \texttt{centro}}{
         ...
         \uIf{$ativacao >= \epsilon$}{
            marque \texttt{centro} como ativo\\
            $\epsilon \leftarrow ativacao$\\ 
         }
      }
    }   
    \Retorna{mudanca}
