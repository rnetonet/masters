...
Para execução dos experimentos deste projeto de mestrado, os parâmetros da tarefa \textit{BasicConceptDriftPerformanceEvaluator} foram configurados conforme a Tabela \ref{tbl:params_basic_concept_drift_evaluator}.

\begin{center} 
    \begin{table}[ht]
    \label{tbl:params_basic_concept_drift_evaluator}
    \begin{tabularx}{\textwidth}{|p{3cm}|p{5cm}|X|}
    \cline{1-3}
    \multicolumn{1}{|c}{\textbf{Parâmetro}} & \multicolumn{1}{|c|}{\textbf{Valor}} & \multicolumn{1}{c|}{\textbf{Observação}}       \\ \cline{1-3}
    \textit{learner}                  & \textit{ChangeDetectorLearner}                 &  O algoritmo de detecção de mudanças de conceito a ser testado é definido no atributo \textit{driftDetectionMethod} da classe \textit{ChangeDetectorLearner}.                   \\ \cline{1-3}
    \textit{stream}                  & \textit{ARFFFileStream}                &  Caminho para um dos arquivos \textit{ARFF} descrito na seção anterior. O atributo \textit{classIndex} deve ser definido como $0$, pois não existem rótulos nestes conjuntos de dados.  \\ \cline{1-3}
    \textit{instanceLimit}                  & $-1$                &  Desabilita o limite de instâncias a serem processadas.  \\ \cline{1-3}
    \textit{timeLimit}                  & $-1$                &  Desabilita o limite de tempo de execução.  \\ \cline{1-3}
    \textit{sampleFrequency}                  & $1$                &  Uma linha de resultado do avaliador deve ser gerada para cada instância processada.  \\ \cline{1-3}
    \cline{1-3}
    \end{tabularx}
...
Neste trabalho de mestrado, serão analisadas as seguintes dimensões de avaliação:
tempo de avaliação (\textit{evaluation time (cpu seconds)}), 
mudanças detectadas (\textit{detected changes}),
mudanças verdadeiras (\textit{true changes}),
atraso de detecção (\textit{delay detection (average)}) e mudanças reais detectadas (\textit{true changes detected}).
