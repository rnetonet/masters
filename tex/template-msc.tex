...
Como resultado, o interesse de pesquisa no gerenciamento de grandes volumes de dados tem crescido.
Parte relevante desses dados é adquirido através de sequências contínuas, potencialmente infinitas e de alta frequência.
A literatura denomina séries com essas características como Fluxos Contínuos de Dados (FCDs) \cite{Aggarwal:2003:FCE:1315451.1315460, Gama:2010:KDD:1855075, GamaMCR04}.
Entretanto, lidar com volumes de dados tão grandes impõe desafios aos pesquisadores, sobretudo limitações de tempo de execução e recursos computacionais.

Conforme \cite{Babcock:2002:MID:543613.543615} as principais diferenças de FCDs para os dados convencionais são:
\begin{itemize}
    \item Tamanho ilimitado;
    \item Dados recepcionados de forma contínua e em alta frequência;
    \item Não há controle sobre a ordem de chegada dos eventos;
    \item Eventos processados devem ser descartados.
\end{itemize}
...
A metodologia descrita é adequada para ambientes cuja distribuição dos dados é estacionária, 
...
Agrupamento é a tarefa de associar objetos conforme sua similaridade.
A técnica tem como objetivo formar grupos com alta similaridade entre seus objetos e baixa similaridade para objetos de outros grupos.
...
% Dentre os importantes algoritmos para agrupamento em cenários batch destacam-se: 
% K-Means (Lloyd,1982)(MacQueen, 1967), 
% DBSCAN (Ester et al., 1996), 
% OPTICS (Ankerst et al., 1999),
% PAM (Kaufman e Rousseeuw, 2005), 
% CURE (Guha et al., 1998), 
% ROCK (Guha et al., 1999),
% CHAMALEON (Karypis et al., 1999) e COBWEB (Fisher, 1987). 
% Uma das possíveis razões para a diversidade de algoritmos de agrupamento é que não há uma definição geral
% de grupos, e em geral, diferentes métodos de agrupamento se adaptam a diferentes tipos
% de aplicação (Kaufman e Rousseeuw, 2005).
...
Segundo \cite{Gama:2010:KDD:1855075}, a principal dificuldade na aplicação de técnicas de agrupamentos em FCDs é definida como:
manter a qualidade do agrupamento e a consistência em relação à sequência de dados observados, utilizando pouca memória e tempo.
Estas dificuldades são impostas pela chegada contínua de dados e a necessidade de produzir respostas em tempo hábil.
