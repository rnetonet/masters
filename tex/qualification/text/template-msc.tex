...
%
Além disso, a técnica foi aplicada ao problema de classificação de fixações e sacadas na ativade de rastreamento ocular:
...
Ademais, a técnica desenvolvida apresentou bons resultados quando aplicada ao problema de monitoramento ocular, pois foi capaz de classificar fixações e sacadas em tempo real e com precisão comparável ao estado da arte.

% Os resultados obtidos com conjuntos de dados sintéticos apresentaram desempenho superior ou similar ao de algoritmos consolidados na literatura, em termos de acurácia média e tempo de processamento.
% Além disso, o método foi aplicado ao problema de classificação de fixações e sacadas na atividade de monitoramento ocular:
% uma questão crítica para diferentes áreas do conhecimento, pois muitos experimentos comportamentais usam informações de rastreamento ocular como um fator de análise relevante.
% A técnica desenvolvida foi capaz de classificar fixações e sacadas em tempo de execução e com acurácia similar ao estado da arte.

% Palavras-chave do resumo em Portugues
\begin{keywords}
    Fluxos Contínuos de Dados, Mudanças de Conceito, Redes de Função de Base Radial, Cadeias de Markov, Monitoramento Ocular
\end{keywords}
...
% where {\displaystyle N} N is the number of neurons in the hidden layer, {\displaystyle \mathbf } {\mathbf  c}_{i} is the center vector for neuron {\displaystyle i} i, and {\displaystyle } a_{i} is the weight of neuron {\displaystyle i} i in the linear output neuron.
