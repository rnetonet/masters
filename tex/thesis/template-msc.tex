...
Entretanto, o desenvolvimento de tecnologias não invasivas de rastreamento favoreceu o aumento da sua utilização.
...
o problema de classificação em sacadas e fixações durante atividades de rastreamento ocular foi discutido.
...
Este capítulo apresenta o RBFChain, um novo método de detecção de mudanças de conceito proposto por esta dissertação.
A técnica desenvolvida é apresentada da seguinte forma:
a Seção \ref{sec:rbfchain_visao_geral} inicia o capítulo com uma visão geral da proposta.
Na Seção \ref{sec:rbfchain_detalhes}, o algoritmo desenvolvido é apresentado em formato pseudocódigo e seu funcionamento é detalhado.
Por fim, são discutidas as considerações finais do Capítulo na Seção \ref{sec:rbfchain_consideracoes_finais}.

\section{Visão Geral}
\label{sec:rbfchain_visao_geral}

% Este trabalho apresenta uma solução que visa minimizar os impactos causados na rede,
% pela mobilidade de produtores na NDN, através do uso oportunista da cache dos nós
% presentes na borda da rede sem fio. Ao usar sua cache, um nó móvel pode até servir como
% meio de rede entre áreas desconectadas ou fornecer conectividade atrasada em enlaces
% esporadicamente conectados (JACOBSON et al., 2009). O objetivo é proporcionar que os
% produtores sejam capazes de replicar seus dados, de forma a manter a maior diversidade
% de conteúdos entre os nós e que estes possam armazenar os dados com o mı́nimo de
% esforço, para que os interesses futuros consigam ser satisfeitos mesmo durante o perı́odo
% de indisponibilidade do produtor.

% A solução se baseia no princı́pio básico de armazenar os dados que trafegam na rede
% sem fio, de modo a evitar as replicações em todos os nós do domı́nio, com o intuito de
% alcançar uma maior diversidade de conteúdos na rede. Isso é possı́vel, pois a NDN possui
% caracterı́sticas como cache na camada de rede e entrega multicast, além de viabilizar
% que os dados possam ser facilmente retirados dos produtores originais, uma vez que a
% proteção é aplicada diretamente sobre os mesmos e não no meio de acesso, como no
% caso da rede IP (ZHANG et al., 2016). A SCaN-Mob se beneficia dessas caracterı́sticas
% da NDN e permite que os dados sejam armazenados sem esforço e oportunisticamente,
% através da comunicação broadcast das redes sem fio. Por outro lado, as retransmissões
% de interesses inundam a rede, devido à caracterı́stica citada do canal sem fio, por isso, a
% solução controla essas retransmissões.
