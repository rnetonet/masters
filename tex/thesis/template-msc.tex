...
Visando superar essas limitações, este trabalho apresenta um novo método de detecção de mudança de conceito, denominado RBFChain, baseado em Redes de Função de Base Radial (RBF) e Cadeias de Markov.
...
De maneira complementar, as cadeias de Markov permitem modelar as transições de centro ocorridas em tais grupos de observações.
Mudanças de conceito são, então, detectadas quando o centro ativo do agrupamento é alterado e a probabilidade da transição, no modelo markoviano, excede um limiar.
...
Este capítulo detalha o método de detecção de mudanças de conceito proposto nesta dissertação, denominado RBFChain.
A técnica desenvolvida é apresentada da seguinte forma:
a Seção \ref{sec:rbfchain_visao_geral} inicia o capítulo com uma visão geral, abordando a arquitetura e o papel desempenhado por cada componente.
...
A arquitetura do RBFChain é composta por dois componentes principais: uma Rede de Função de Base Radial (RBF) e uma Cadeia de Markov.
...
As mudanças de centro desse cluster são modeladas através da Cadeia de Markov.
...
Ao observar um novo evento, este é submetido ao processo de ativação da rede RBF.
Durante o processo, o valor de ativação do evento é calculado para cada centro da rede, através da função Gaussiana.
O evento é, então, vinculado ao centro com maior valor de ativação, desde que esse valor seja maior que $\lambda$.
Se não for possível vincular o evento a nenhum dos centros, o próprio evento se torna o novo centro ativo da rede.

As ativações de novos ou diferentes centros são modeladas pela Cadeia de Markov,

A Cadeia de Markov é responsável pela modelagem das mudanças de centro ativo,
aumentando a probabilidade da transição ocorrida pelo fator $\alpha$ e diminuindo proporcionalmente o valor das outras transições possíveis para o estado originário.
