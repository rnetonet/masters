...
%
Por fim, o problema de classificação de fixações e sacadas em atividades de rastreamento ocular é detalhado.
...
Além de modelar o processo em execução, a Cadeia de Markov também é responsável pela tolerância a ruídos do método de detecção proposto neste trabalho.
Isto ocorre, pois apenas transições consideradas \textit{efetivas} ($P_{ij} = 1$) podem sinalizar mudanças de conceito.
...
A análise rigorosa dos movimentos oculares não é uma atividade experimental recente, remontando ao seminal trabalho de \citeonline{Yarbus1967}.
Entretanto, o desenvolvimento de tecnologias não invasivas de rastreamento ocular por infravermelho favoreceu a utilização dessa técnica.
Atualmente, a técnica de rastreamento ocular é utilizada em diferentes tarefas experimentais \cite{Duchowski_EyeTrackingSurvey}, como, por exemplo:
localização da atenção visual,
detecção de deficiências cognitivas,
avaliação de estratégias de pesquisa visual, entre outras.
Estas tarefas produzem informações utilizadas como fatores de análise para um vasta gama de pesquisas em diferentes áreas do conhecimento.
...
% Neste capítulo foram apresentados os principais conceitos de séries temporais e compo-
% nentes que descrevem o comportamento de suas observações. Com base nesses compor-
% tamentos, foram discutidos modelos disponibilizados pela área Estatística e de Sistemas
% dinâmicos, cujo principal objetivo é compreender o comportamento de séries temporais.
% Além disso, foram discutidos trabalhos que modelam séries temporais no domínio de
% frequências e técnicas que utilizam ferramentas disponibilizadas pela área de Aprendizado
% de Máquina. Como pôde ser observado, esses trabalhos e técnicas possuem limitações,
% principalmente quando o objetivo é realizar a modelagem segundo os componentes esto-
% cásticos e determinísticos. Essas limitações motivaram o desenvolvimento deste trabalho,
% que é detalhado no próximo capítulo.
...
% 30
%  REDES CENTRADAS NA INFORMAÇÃO
